...
- **d10** (v4 traction-fix, 2026-05-18): user reported zero traction after 4 days (1 star, 6 views all day-one). Three parallel expert agents (growth, PM, deep research) converged on overlapping root causes. Implemented "Tier A" — the evidence-overwhelming, no-strategy-decision fixes:
  - `.claude-plugin/marketplace.json` added — the registry manifest auto-crawled by skills.sh / claudemarketplaces / claudeskills.info. Without it the repo was structurally invisible to the dominant discovery channel.
  - `scripts/publish.py` — auto-submit to awesome-lists is now **OFF by default**. New `--auto-submit-prs` flag for opt-in. travisvn/awesome-claude-skills CONTRIBUTING explicitly closes AI-generated and sub-10-star PRs; the prior default actively burned user reputation with the curators they need. PR bodies still saved to memory/pr-bodies.md for manual submission post-traction.
  - `SKILL.md` Phase D + Phase E rewritten to reflect the new default (no auto-submit; bodies saved; "Never frame the default as a failure" hard rule).
  - `README.md` rewritten: badges + cross-CLI moved to top of file (was buried in footer); hero changed from cargo-cult "structured like top installs" → honest "researches GitHub before you build"; new "Why no auto-submit" section explains the default. Word count 537 (within readme-rules.md 312-668 tolerance).
  - Tier B (rename, demo GIF, coordinated launch) and Tier C (strategic pivot to recon-only grader) deferred pending user decision.
