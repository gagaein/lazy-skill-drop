...
### Identity invariants — verify before merging
This PR must NOT cause any of the three core innovations to disappear from README.md or SKILL.md. The mission anchor is `references/core-identity.md` (frozen, never auto-edited).

1. **Recon** — analyze adjacent competitors before writing
2. **Forge** — generate README.md + SKILL.md from this week's viral formula
3. **Publish** — one user confirmation, then automatic `gh repo create` + awesome-list PRs

`scripts/identity_guard.py` runs at the top of every evolve workflow tick and will already have passed for this branch. Re-run it locally before merging if you are not sure: `python scripts/identity_guard.py`.
